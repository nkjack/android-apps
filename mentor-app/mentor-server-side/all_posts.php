...
if (isset($post['dbid_degree'])&&isset($post['amount_to_fetch'])&&isset($post['from_row'])&&isset($post['type_post'])
			&&isset($post['unique_id']))
{

	$amount_to_fetch= intval($post['amount_to_fetch']);
	$from_row= intval($post['from_row']);
	$degree_id = intval($post['dbid_degree']);
	...
	$query_all = "SELECT DISTINCT sub3.dbid_post,
						...
						(
							...
							(
								...
								JOIN tb_degrees dg
								ON dg.dbid_degree = ps.degree_id
								JOIN tb_faculties fc
								ON fc.dbid_faculty = dg.faculty_id
								JOIN tb_schools sc
								ON sc.dbid_school = fc.school_id
								where ps.degree_id = '$degree_id'
								and ps.type_post ='$type_post'
							) sub2
							LEFT JOIN tb_comments_posts ps		ON ps.uid_post = sub2.dbid_post
							LEFT JOIN tb_posts_votes_user vs	ON (vs.uid_user = '$unique_id' AND
