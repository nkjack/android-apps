...
import com.studentadvisor.noam.studentadvisor.adapters.AdapterExploreFeeds;
...
import com.studentadvisor.noam.studentadvisor.tasks.TaskLoadExplorePosts;
 ...
 */
public class ExploreFeedsFragment extends Fragment implements SwipeRefreshLayout.OnRefreshListener,
        PostsLoadedListener {
    private DialogCommunicatorNewsFeed mCommunicator;

    private RecyclerView recyclerView;
    private ProgressBar mProgressBar;
    private AdapterExploreFeeds rvAdapter;
    ...
    private static final String STATE_POSTS_EXPLORE_FEEDS = "state_posts_explore_feeds";
    private static final String STATE_ROWS_TO_FETCH_EXPLORE = "rows_to_fetch_explore";
    private static final String STATE_START_FROM_ROW_EXPLORE = "start_from_row_explore";
    ...
    private ExploreFeedsFragment fragmentContext;

    private int mTabFragmentNumber = 3;
    ...
    public static ExploreFeedsFragment newInstance(int tab_num) {
        ExploreFeedsFragment fragment = new ExploreFeedsFragment();
...
//        start_from_row = 0;
        ...
        if (savedInstanceState != null) {
            rows_to_fetch = savedInstanceState.getInt(STATE_ROWS_TO_FETCH_EXPLORE);
            start_from_row = savedInstanceState.getInt(STATE_START_FROM_ROW_EXPLORE);
        }
...
//        onRefresh();
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        View view = inflater.inflate(R.layout.fragment_explore_feeds, container, false);
        mRoot = (RelativeLayout) view.findViewById(R.id.myExploreFeedsFragmentRoot);

        mSwipeRefreshLayout = (SwipeRefreshLayout) view.findViewById(R.id.swipeExploreFeedsRefresh);
        mSwipeRefreshLayout.setOnRefreshListener(this);
        recyclerView = (RecyclerView) view.findViewById(R.id.myExploreFeedsRecycle);
        recyclerView.addItemDecoration(new MarginDecoration(getActivity()));

        rvAdapter = new AdapterExploreFeeds(getActivity());
                        ...
                        if (post != null) {
                            ...
                            if (session.isLoggedIn()) {
                                ...
                                if (mCommunicator.CheckEnabledInternet()) {
                                    ...
                                    mCommunicator.updatePostsActionButton(post.getDbid_post(), true, false);
                                }
                            } else {
                                // Log in Dialog
...
//                                        post.getDbid_post();
                                    ...
                                    rvAdapter.notifyDataSetChanged();
                                    mCommunicator.updatePostsActionButton(post.getDbid_post(), false, false);
...
//                mCommunicator.dialog_post_comments();
                    ...
                    public void onClickFetchMoreButton(View view, int position) {
                        ...
                        if (mUserUid != null && mContextPostsListener != null && position > 0) {
                            if (!isLoading && !isFinishFetch && !isFetching && mCommunicator.CheckEnabledInternet()) {
                                ...
                                new TaskLoadExplorePosts(mContextPostsListener, !FIRSTS_POSTS,
                                        mUserUid)
                                        .execute();
        ...
        mProgressBar = (ProgressBar) view.findViewById(R.id.myExploreFeedsProgressBar);

        if (mCommunicator != null && !mCommunicator.getListExplorePost().isEmpty()) {

            data = mCommunicator.getListExplorePost();
            rvAdapter.setPosts(data);
            recyclerView.getAdapter().notifyItemRangeChanged(0, data.size() - 1);

        } else if (savedInstanceState != null && savedInstanceState.getParcelableArrayList(STATE_POSTS_EXPLORE_FEEDS) != null) {
            data = savedInstanceState.getParcelableArrayList(STATE_POSTS_EXPLORE_FEEDS);
            rvAdapter.setPosts(data);
        }
        else {
            if (mUserUid != null) {
                if (mCommunicator.CheckEnabledInternet()) {
                    new TaskLoadExplorePosts(this, FIRSTS_POSTS,
                            mUserUid)
                            .execute();
                }
            } else {
    ...
    public void onSaveInstanceState(Bundle outState) {
        ...
        outState.putParcelableArrayList(STATE_POSTS_EXPLORE_FEEDS, data);
        outState.putInt(STATE_ROWS_TO_FETCH_EXPLORE, rows_to_fetch);
        outState.putInt(STATE_START_FROM_ROW_EXPLORE, start_from_row);
    }
    ...
    public void onPostsLoadedVolley(ArrayList<Post> listPosts) {
        ...
        if (!listPosts.isEmpty()) {
            ...
            if (mCommunicator != null) {
                mCommunicator.setListExplorePosts(listPosts, false);
            }
        }
    }
...
//        recyclerView.getAdapter().notifyItemRangeChanged(0, data.size() - 1);
        ...
        if (mCommunicator != null) {
            mCommunicator.setListExplorePosts(listPosts, true);
        }
